...
addTask.addEventListener('click', function(){                             // Если поля не заполнены
...
function displayTasks(){                                  //Создает элементы списка
    let displayTask = '';
    ...
    todoList.forEach(function(item, i){
        ...
        <li>
            ...
            <label for='item_${i}'  class="${item.priority == 1 ? 'priority1' : (item.priority == 2 ? 'priority2' : '')}">${item.todo}</label>
...
todo.addEventListener('contextmenu', (e) => {              //Удаление задания Правой кнопкой мыши!!!
    e.preventDefault();
    todoList.forEach(function(item, i){
